...
use cursive::views::{ResizedView, TextArea, TextView};
use cursive::Cursive;
...
fn main() {
    ...
    let _ = mux
        .add_right_of(
            cursive::views::Panel::new(ResizedView::with_full_screen(TextArea::new())),
            top_right_mid,
        )
        .unwrap();
    let _ = mux
    ...
    let idlayer = cursive::views::NamedView::new("Steven", mux);
...
fn add_plane(siv: &mut Cursive, node: Id) {
    let mut foo: cursive::views::ViewRef<Mux> = siv.find_name("Steven").unwrap();
