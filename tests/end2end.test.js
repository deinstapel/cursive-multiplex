...
it('tests resizing panes in a complex setup', cargo_e2e('complex_resize', async () => {
    await expect.command('tmux send-keys -N 4 C-Down && sleep 0.1')
        .forExitCode(exp => exp.toBe(0));
    await expect.command('tmux send-keys -N 6 C-Right && sleep 0.1')
        .forExitCode(exp => exp.toBe(0));
    await expect.command('tmux send-keys Left Down && sleep 0.1')
        .forExitCode(exp => exp.toBe(0));
    await expect.command('tmux send-keys -N 4 C-Up && sleep 0.1')
        .forExitCode(exp => exp.toBe(0));
    await expect.command('tmux send-keys -N 6 C-Left && sleep 0.1')
        .forExitCode(exp => exp.toBe(0));
}));
...
it('tests resizing a pane in a horizontal setup', cargo_e2e('horizontal_resize', async () => {
    await expect.command('tmux send-keys -N 20 C-Left && sleep 0.1')
        .forExitCode(exp => exp.toBe(0));
    await expect.command('tmux send-keys -N 5 C-Right && sleep 0.1')
        .forExitCode(exp => exp.toBe(0));}));
...
it('tests resizing a pane in a vertical setup', cargo_e2e('vertical_resize', async () => {
    await expect.command('tmux send-keys -N 4 C-Up && sleep 0.1')
        .forExitCode(exp => exp.toBe(0));
    await expect.command('tmux send-keys -N 2 C-Down && sleep 0.1')
        .forExitCode(exp => exp.toBe(0));
}));
